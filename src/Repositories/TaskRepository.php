...
class TaskRepository
{
    ...
    public function taskHasProof(int $taskId, int $uploadedBy, array $types): bool
    {
        if ($types === []) {
            return false;
        }
        ...
        $sql = "SELECT COUNT(*) FROM task_proofs WHERE task_id = ? AND uploaded_by = ? AND proof_type IN ($placeholders)";
        $stmt = $this->pdo->prepare($sql);
        $params = array_merge([$taskId, $uploadedBy], $types);
        $stmt->execute($params);
