...
class TaskStateGuard
{
    public function assertTransitionAllowed(array $task, int $actorId, string $actorRole, string $targetState): void
    {
        ...
        switch ($currentState) {
            case 'posted':
                if ($targetState === 'accepted') {
                    if (!in_array($actorRole, ['runner', 'both'], true)) {
                        throw new RuntimeException('Only runners can accept posted tasks.');
                    }
                    return;
                }

                if ($targetState === 'cancelled') {
                    if ((int) $task['client_id'] !== $actorId) {
                        throw new RuntimeException('Only task client can cancel a posted task.');
                    }
                    return;
                }
            ...
            case 'disputed':
                if ($targetState !== 'completed' || $actorId !== 1) {
                    throw new RuntimeException('Only admin can close disputed tasks.');
                }
                return;
