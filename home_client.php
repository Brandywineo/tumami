...
<section class="section section--compact app-listing app-shell-page">
    <div class="container container--mobile-dense app-shell__container">
        <article class="card card--compact app-shell__header">
            <p class="dashboard-subtitle" style="margin:0;">Good day 👋</p>
            <h1 class="dashboard-title">Home</h1>
        </article>

        <div class="grid app-stats-grid">
            <article class="card card--compact"><p class="dashboard-subtitle">Active Errands</p><p class="stat-value"><?php echo $activeCount; ?></p></article>
            ...
            <article class="card card--compact"><p class="dashboard-subtitle">Recent</p><p class="stat-value"><?php echo count($recent); ?></p></article>
        </div>

        <article class="card card--compact app-shell__primary-cta">
            <a href="post_task.php" class="cta-button cta-button--block">Post a New Errand</a>
        </article>

        <article class="card card--compact app-listing__section">
            <div class="app-listing__section-header">
                <h3>Recent Errands</h3>
                <a href="client_errands.php">View all</a>
            </div>
            <div class="grid grid--dashboard">
                <?php if ($recent === []): ?>
                    <p class="list-empty">No errands yet. Create your first errand.</p>
                <?php endif; ?>
                <?php foreach ($recent as $task): ?>
                    <article class="card card--compact">
                        <h3><?php echo h((string) ($task['title'] ?? 'Untitled errand')); ?></h3>
                        <p><strong>Status:</strong> <?php echo h((string) ($task['status'] ?? 'posted')); ?></p>
