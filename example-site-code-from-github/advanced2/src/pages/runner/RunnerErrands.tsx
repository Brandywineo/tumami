...
const RunnerErrands = () => {
  ...
  const loadErrands = async () => {
    if (!user) return;
    const { data } = await supabase.from("errands").select("*").eq("runner_id", user.id).order("created_at", { ascending: false });
    setErrands(data || []);
    setLoading(false);
  };
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("runner-errands-realtime")
      .on("postgres_changes", { event: "*", schema: "public", table: "errands", filter: `runner_id=eq.${user.id}` }, (payload) => {
        ...
        {filtered.length === 0 ? (
          <div className="p-8 rounded-2xl bg-card border border-border text-center">
            ...
            <p className="text-sm text-muted-foreground">
              {errands.length === 0 ? "No errands accepted yet. Browse available errands from your dashboard." : "No errands match your filters."}
            </p>
          </div>
        ) : (
