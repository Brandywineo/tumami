...
import { Shield, Upload, CheckCircle, Clock, XCircle, ArrowLeft, Loader2, AlertTriangle, X, Info } from "lucide-react";
...
const RunnerVerification = () => {
  ...
  const [form, setForm] = useState({
    ...
    transport: "", emergency_contact: "", availability: "full_time", bio: "", username: "",
  });
  ...
  const uploadFile = async (file: File, folder: string): Promise<string> => {
    ...
    const path = `runner-verification/${user!.id}/${folder}-${Date.now()}.${ext}`;
  ...
  const handleSubmit = async () => {
    if (!user) return;
    if (!form.full_name || !form.phone || !form.national_id || !form.town || !form.transport || !form.username) {
      toast.error("Please fill in all required fields including username");
      return;
    }
    if (!/^[a-zA-Z0-9_.]{3,30}$/.test(form.username)) {
      toast.error("Username must be 3-30 characters: letters, numbers, underscores, dots only");
      return;
    }
    ...
    try {
      ...
      const [idFrontUrl, idBackUrl, selfieUrl, goodConductUrl] = await Promise.all([
        uploadFile(files.idFront!, "id-front"),
        uploadFile(files.idBack!, "id-back"),
        uploadFile(files.selfie!, "selfie"),
        uploadFile(files.goodConduct!, "good-conduct"),
      ]);
      ...
      const payload = {
        ...
        profile_photo_url: goodConductUrl,
      ...
      try {
        ...
        if (prof?.email) {
          await supabase.functions.invoke("send-transactional-email", {
            body: {
              templateName: "errand-created",
              recipientEmail: prof.email,
              idempotencyKey: `runner-app-submitted-${user.id}-${Date.now()}`,
              templateData: { name: form.full_name, title: "Runner Application Submitted", description: "Your runner application has been submitted and is under review. We'll notify you once it's been processed." },
            },
          });
        }
      } catch { /* non-blocking */ }
    } catch (err: any) {
      if (err.message?.includes("idx_runner_verifications_username")) {
        toast.error("Username is already taken. Please choose another.");
      } else {
      ...
      <AppLayout type="runner">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 pt-8">
          ...
          <div className="p-5 rounded-2xl bg-card border border-border shadow-premium space-y-4">
            ...
            <SummaryRow label="Username" value={existing?.username ? `@${existing.username}` : undefined} />
    ...
    return (
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          <Input value={form.username} onChange={e => update("username", e.target.value.toLowerCase().replace(/[^a-z0-9_.]/g, ""))} placeholder="peter.runner" className="h-12 rounded-xl" maxLength={30} />
          <div className="flex items-center gap-1.5">
            {usernameStatus === "checking" && <Loader2 className="w-3 h-3 text-muted-foreground animate-spin" />}
            {usernameStatus === "available" && <CheckCircle className="w-3 h-3 text-primary" />}
            {usernameStatus === "taken" && <XCircle className="w-3 h-3 text-destructive" />}
            <p className={`text-[10px] ${usernameStatus === "taken" ? "text-destructive" : usernameStatus === "available" ? "text-primary" : "text-muted-foreground"}`}>
              {usernameStatus === "checking" ? "Checking availability..." :
               usernameStatus === "available" ? "Username is available!" :
               usernameStatus === "taken" ? "Username is already taken" :
               "3-30 characters. Letters, numbers, dots, underscores. Clients can search you by this username."}
            </p>
          </div>
        </div>
        <div className="space-y-2">
        ...
        <div>
          ...
          <p className="text-[10px] text-muted-foreground mt-2">Tap to upload files from your device. Files are securely stored.</p>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-primary/5 border border-primary/20">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-foreground mb-1">What clients can see</p>
              <p className="text-xs text-muted-foreground leading-relaxed">
                Some profile details such as your display name, username, bio, rating, and completed errands will be visible to clients so they can identify and choose you.
              </p>
              <p className="text-xs text-muted-foreground mt-2 leading-relaxed">
                Your ID documents, phone number, national ID number, and verification files are <span className="font-semibold text-foreground">never shown publicly</span>.
              </p>
            </div>
          </div>
        </div>

        <Button className="w-full h-12 rounded-xl font-semibold gap-2" onClick={handleSubmit} disabled={submitting || !allFilesUploaded || usernameStatus === "taken"}>
