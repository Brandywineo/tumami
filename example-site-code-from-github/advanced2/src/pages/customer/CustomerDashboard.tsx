...
import {
  Plus, Package, CheckCircle, Clock, Wallet, ArrowRight, Loader2, MessageCircle, Search,
} from "lucide-react";
import { motion } from "framer-motion";
import { useAuth } from "@/contexts/AuthContext";
import { supabase } from "@/integrations/supabase/client";
import PreferredRunners from "@/components/runner/PreferredRunners";
import RunnerSearch from "@/components/runner/RunnerSearch";
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const { data: convos } = await supabase.from("conversations")
        .select("id").or(`participant_1.eq.${user.id},participant_2.eq.${user.id}`);
    ...
    <AppLayout type="customer">
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Link to="/customer/messages" className="relative p-2 rounded-xl bg-muted hover:bg-muted/80 transition-colors">
