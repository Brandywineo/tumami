...
const WithdrawalApprovedEmail = ({ name, amount }: Props) => (
  <Html lang="en" dir="ltr">
    <Head />
    <Preview>Your withdrawal of KES {amount?.toLocaleString() || '0'} has been approved</Preview>
    <Body style={main}>
      <Container style={container}>
        <Heading style={{ ...h1, color: BRAND }}>Withdrawal Approved</Heading>
        <Text style={text}>{name ? `Hi ${name},` : 'Hi,'}</Text>
        <Text style={text}>Your withdrawal request of <strong>KES {amount?.toLocaleString() || '0'}</strong> has been approved and is being processed.</Text>
        <Text style={text}>You'll receive the funds via M-Pesa shortly.</Text>
...
export const template = {
  ...
  subject: (data: Record<string, any>) => `Withdrawal Approved: KES ${data.amount?.toLocaleString() || '0'} — ${SITE}`,
