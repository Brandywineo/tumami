...
import { Wallet as WalletIcon, ArrowUpRight, ArrowDownLeft, Plus, ArrowDown, X, Loader2, TrendingUp, Calendar, Award, AlertCircle } from "lucide-react";
...
const WalletPage = ({ type = "customer" }: { type?: "customer" | "runner" }) => {
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      // Refresh withdrawals
      if (type === "runner") {
        const { data: w } = await supabase.from("runner_withdrawals").select("*").eq("user_id", user.id).order("requested_at", { ascending: false }).limit(20);
        setWithdrawals(w || []);
      } else {
        const { data: w } = await supabase.from("withdrawal_requests").select("*").eq("user_id", user.id).order("created_at", { ascending: false }).limit(20);
        setWithdrawals(w || []);
      }
    } catch (err: any) {
              ...
              {withdrawals.map(w => (
                <div key={w.id} className="flex items-center justify-between p-3 rounded-xl bg-card border border-border">
                  ...
                  <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full capitalize ${
                    w.status === "paid" ? "bg-emerald-light text-primary" :
            ...
            {filtered.map((t, i) => {
              ...
              return (
                <motion.div key={t.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03, duration: 0.3 }}
                  className="flex items-center gap-3 p-4 rounded-2xl bg-card border border-border shadow-premium">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${isCredit ? "bg-emerald-light" : "bg-accent/10"}`}>
                  ...
                  <span className={`text-sm font-bold flex-shrink-0 ${isCredit ? "text-primary" : "text-foreground"}`}>
