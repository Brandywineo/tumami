...
import { Loader2, Wallet, CheckCircle, XCircle, Banknote } from "lucide-react";
...
const WithdrawalsTab = () => {
  ...
  const fetchData = async () => {
    ...
    const [oldRes, newRes] = await Promise.all([
      supabase.from("withdrawal_requests").select("*").in("status", ["pending", "approved"]).order("created_at", { ascending: false }),
      supabase.from("runner_withdrawals").select("*").in("status", ["pending", "approved"]).order("requested_at", { ascending: false }),
    ]);
    const combined = [
            ...
            <Card className="rounded-2xl border-border shadow-premium">
              <CardContent className="p-4 space-y-3">
                <div className="flex items-start justify-between">
                  ...
                  <Badge variant={w.status === "approved" ? "default" : "secondary"} className="text-[10px] capitalize">{w.status}</Badge>
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  {w.status === "approved" && (
                    <Button size="sm" className="flex-1 rounded-xl gap-1" disabled={acting === w.id} onClick={() => doAction("mark_paid", w.id)}>
